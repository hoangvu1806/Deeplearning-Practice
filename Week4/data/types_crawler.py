...
def crawl_categories(categories, total_pages, urls_dpath, results_dpath):
    """"
    Crawl articles from specified categories with total_pages per category
    @param categories (dict): dictionary of categories with {name: url_slug}
    @param total_pages (int): number of pages to crawl
    @param urls_dpath (str): path to urls directory
    @param results_dpath (str): path to results directory
    @return total_error_urls (list(str)): list of error urls
    ...
    for name, article_type in categories.items():
        print(f"Crawling category: {name} ({article_type})")
        
        # Tạo thư mục cho từng category
        results_category_dpath = os.path.join(results_dpath, article_type)
        create_dir(results_category_dpath)
        
        # Lấy danh sách URL
        articles_urls = get_urls_of_type(article_type, total_pages)
        if not articles_urls:
            print(f"Không tìm thấy bài viết nào cho category: {name} ({article_type})")
            continue
            
        # Lưu danh sách URL
        articles_urls_fpath = os.path.join(urls_dpath, f"{article_type}.txt")
        with open(articles_urls_fpath, "w", encoding="utf-8") as urls_file:
            urls_file.write("\n".join(articles_urls))
        
        # Crawl các URL trong category
        error_urls = crawl_urls(articles_urls_fpath, results_category_dpath)
        total_error_urls.extend(error_urls)
    
    return total_error_urls

def main(article_type, all_types=False, categories=None, total_pages=1, output_dpath="data"): 
    urls_dpath, results_dpath = _init_dirs(output_dpath)
    error_urls = list()

    if categories:
        error_urls = crawl_categories(categories, total_pages, urls_dpath, results_dpath)
    elif all_types:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--categories",
                        default=None,
                        help="use custom categories dictionary",
                        action="store_true",
                        dest="use_categories")
    ...
    # Sử dụng categories tùy chỉnh nếu được yêu cầu
    ...
    # Danh sách chuyên mục cố định
        categories = {
            ...
            # "Thời sự": "thoi-su",
            # "Pháp luật": "phap-luat",
            "Khoa học công nghệ": "khoa-hoc-cong-nghe/the-gioi-tu-nhien",
            # "Sức khỏe": "suc-khoe",
            # "Giải trí": "giai-tri",
            # "Thể thao": "the-thao",
        }
        args_dict = vars(args)
        args_dict["categories"] = categories
        args_dict.pop("use_categories")
        main(**args_dict)
    else:
        main(**vars(args))
